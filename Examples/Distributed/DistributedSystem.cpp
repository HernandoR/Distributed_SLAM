...
namespace ORB_SLAM3
{
    ...
    Disti_System::Disti_System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
...
//             << endl;
        ...
        {
            settings_ = nullptr;
            cv::FileNode node = fsSettings["Disti_System.LoadAtlasFromFile"];
            if (!node.empty() && node.isString())
            {
                mStrLoadAtlasFromFile = (string)node;
            }

            node = fsSettings["Disti_System.SaveAtlasToFile"];
            if (!node.empty() && node.isString())
            {
        ...
        else
        {
            ...
            { // Load ORB Vocabulary
                ...
                mpVocabulary = new ORBVocabulary();
                // this could be optimised, i saw examples
                bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile);
                if (!bVocLoad)
                {
                    cerr << "Wrong path to vocabulary. " << endl;
            ...
            // Create KeyFrame Database
            // TODO what does DB use for? Why can constrcut from voc
            mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);
...
// TODO
        ...
        // has constactor and destracter, it's a class!
        ...
        // such a thread running ...run(mpLocalMapper)
        // though run does not actually need a arg, this is more like mpLocalMapper.Run()
        // i think this may let to problems
        mptLocalMapping = new thread(&ORB_SLAM3::LocalMapping::Run, mpLocalMapper);

        //why it can set after running?
    ...
    Sophus::SE3f Disti_System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        ...
        if (mSensor == Disti_System::IMU_STEREO)
    ...
    Sophus::SE3f Disti_System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        ...
        if (mSensor == Disti_System::IMU_RGBD)
            for (size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
                mpTracker->GrabImuData(vImuMeas[i_imu]);
    ...
    Sophus::SE3f Disti_System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        ...
        // Check reset
        {
            ...
            {
                cout << "Disti_System-> Reseting active map in monocular case" << endl;
                mpTracker->ResetActiveMap();
                mbResetActiveMap = false;
            }
        }

        if (mSensor == Disti_System::IMU_MONOCULAR)
            for (size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
                mpTracker->GrabImuData(vImuMeas[i_imu]);
    ...
    void Disti_System::ActivateLocalizationMode()
    {
        unique_lock<mutex> lock(mMutexMode);
        mbActivateLocalizationMode = true;
    }

    void Disti_System::DeactivateLocalizationMode()
    {
        unique_lock<mutex> lock(mMutexMode);
        mbDeactivateLocalizationMode = true;
    }

    bool Disti_System::MapChanged()
    {
        static int n = 0;
    ...
    void Disti_System::Reset()
    {
        unique_lock<mutex> lock(mMutexReset);
        mbReset = true;
    }

    void Disti_System::ResetActiveMap()
    {
        unique_lock<mutex> lock(mMutexReset);
        mbResetActiveMap = true;
    }

    void Disti_System::Shutdown()
    {
        {
            unique_lock<mutex> lock(mMutexReset);
            mbShutDown = true;
    ...
    bool Disti_System::isShutDown()
    {
        unique_lock<mutex> lock(mMutexReset);
        return mbShutDown;
    }

    void Disti_System::SaveTrajectoryTUM(const string &filename)
    {
        cout << endl
    ...
    void Disti_System::SaveKeyFrameTrajectoryTUM(const string &filename)
    {
        cout << endl
    ...
    void Disti_System::SaveTrajectoryEuRoC(const string &filename)
    {
    ...
    void Disti_System::SaveTrajectoryEuRoC(const string &filename, Map *pMap)
    {
    ...
    /*void Disti_System::SaveKeyFrameTrajectoryEuRoC_old(const string &filename)
    {
    ...
    void Disti_System::SaveKeyFrameTrajectoryEuRoC(const string &filename)
    {
        cout << endl
             << "Saving keyframe trajectory to " << filename << " ..." << endl;
    ...
    void Disti_System::SaveKeyFrameTrajectoryEuRoC(const string &filename, Map *pMap)
    {
        cout << endl
    ...
    /*void Disti_System::SaveTrajectoryKITTI(const string &filename)
    {
    ...
    void Disti_System::SaveDebugData(const int &initIdx)
    {
    ...
    int Disti_System::GetTrackingState()
    {
        unique_lock<mutex> lock(mMutexState);
        return mTrackingState;
    }

    vector<MapPoint *> Disti_System::GetTrackedMapPoints()
    {
        unique_lock<mutex> lock(mMutexState);
        return mTrackedMapPoints;
    }

    vector<cv::KeyPoint> Disti_System::GetTrackedKeyPointsUn()
    {
        unique_lock<mutex> lock(mMutexState);
        return mTrackedKeyPointsUn;
    }

    double Disti_System::GetTimeFromIMUInit()
    {
    ...
    bool Disti_System::isFinished()
    {
        return (GetTimeFromIMUInit() > 0.1);
    }

    void Disti_System::ChangeDataset()
    {
    ...
    float Disti_System::GetImageScale()
    {
        return mpTracker->GetImageScale();
    }

#ifdef REGISTER_TIMES
    void Disti_System::InsertRectTime(double &time)
    {
        mpTracker->vdRectStereo_ms.push_back(time);
    }

    void Disti_System::InsertResizeTime(double &time)
    {
        mpTracker->vdResizeImage_ms.push_back(time);
    }

    void Disti_System::InsertTrackTime(double &time)
    {
        mpTracker->vdTrackTotal_ms.push_back(time);
    }
#endif

    void Disti_System::SaveAtlas(int type)
    {
        if (!mStrSaveAtlasToFile.empty())
        {
            // clock_t start = clock();
    ...
    bool Disti_System::LoadAtlas(int type)
    {
    ...
    string Disti_System::CalculateCheckSum(string filename, int type)
    {
        string checksum = "";
