...
namespace ORB_SLAM3 {
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        cout << "\t-Original image size: [ " << originalImSize_.width << " , " << originalImSize_.height << " ]" << endl;
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        if(!settings.vPinHoleDistorsion1_.empty()){
            ...
            output << "\t-Camera 1 has no distortion parameter" << endl;
        }

        output <<settings.sensor_<<endl;

        output <<System::STEREO<<" "<<"System::IMU_STEREO" <<endl;

        if(settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO){
         
            cout<<" log pior to crash"<<endl;
            output << "\t-Camera 2 parameters (";
            // cout<<" log after point "<< 1 <<endl;
            if(settings.cameraType_ == Settings::PinHole || settings.cameraType_ ==  Settings::Rectified){
                // cout<<" log after point "<< 2 <<endl;
                
                output << "Pinhole";
                cout<<" log after point "<< 3 <<endl;
            }
            else{

                output << "Kannala-Brandt";
            }
            // cout<<" log after point "<< 4 <<endl;
            // output << "" << ": [" << endl ;
            // TODO here is the problem that the camaro type is Rectified
            // such that there is no such thing as settings.originalCalib2_
